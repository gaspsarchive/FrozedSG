...
public class PvPCountdown extends BukkitRunnable {
    public int seconds = GameManager.getInstance().getPvpCountdownValue();
    private long expire;
    ...
    public void everySeconds() {
        if (this.seconds > 60) {
            Utils.broadcastMessage(PotSG.getInstance().getConfiguration("messages").getString("pvp-countdown-minutes").replaceAll("<minutes>", String.valueOf(this.seconds / 60)), true);
        } else {
            Utils.broadcastMessage(PotSG.getInstance().getConfiguration("messages").getString("pvp-countdown-seconds").replaceAll("<seconds>", String.valueOf(this.seconds)), true);
        }
