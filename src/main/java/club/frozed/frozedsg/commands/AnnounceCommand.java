...
public class AnnounceCommand extends BaseCommand {
    ...
    @Command(
            name = "announce",
            permission = "frozedsg.announce",
            aliases = {"ann", "sgannounce"}
    )
    public void onCommand(CommandArgs command) {
        ...
        } else if (!PotSG.getInstance().getConfiguration("config").getBoolean("ANNOUNCE.ENABLED")) {
            player.sendMessage(Color.translate("&cAnnouncing PotSG is not enabled!"));
        } else if (PotSG.getInstance().getAnnounceCooldown() != null && !PotSG.getInstance().getAnnounceCooldown().hasExpired()) {
