...
public class GameCommand extends BaseCommand {
    ...
    @Command(
            name = "game",
            permission = "frozedsg.command.game.use"
    )
    public void onCommand(CommandArgs command) {
        ...
        if (args.length == 0) {
            ...
            if (args.length == 1) {
                switch (args[0]) {
                    ...
                    case "chest":
                        ...
                        break;
                    case "map":
                        Location loc = new Location(WorldsManager.getInstance().getGameWorld(), 0.0, 90.0, 0.0);
                        player.teleport(loc);
                    ...
                    case "setgamespawn":
                        if (!player.getWorld().getName().equalsIgnoreCase(WorldsManager.getInstance().getGameWorld().getName())) {
                            Clickable clickable = new Clickable("&cYou are only allowed to execute this argument in game world. Please use '/game map' to teleport to it or click this message.", "&fClick to teleport to game map", "/game map");
            ...
            if (args.length == 2) {
                if ("spawn".equals(args[0]) && args[1].equalsIgnoreCase("darena")) {
