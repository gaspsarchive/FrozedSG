...
public class DataCommand extends BaseCommand
{
    ...
    public void onCommand(final CommandArgs command) {
        ...
        if (args.length < 4) {
            ...
            player.sendMessage(Color.translate("&8"));
            LeaderboardManager.getInstance().getLeaderboardList().forEach(leaderboard -> player.sendMessage(Color.translate("&7 * &b/data <player> set " + leaderboard.getMongoValue() + " <number> &7- &fset players " + leaderboard.getMongoValue() + "&7.")));
            player.sendMessage(Color.translate("&7&m-------------------------------------"));
        }
        if (args.length == 4) {
            ...
            final PlayerData targetData = PlayerDataManager.getInstance().getByUUID(target.getUniqueId());
            if (targetData == null) {
            ...
            if (!this.isMongoValueAvailable(args[2])) {
    ...
    private boolean isMongoValueAvailable(final String value) {
