...
public class ReloadConfigCommand extends BaseCommand {
    ...
    public void onCommand(CommandArgs command) {
        ...
        PotSG.getInstance().getFiles().forEach((configFile) -> {
            names.add(configFile.getName() + ".yml");
        });
        command.getPlayer().sendMessage(Color.translate("&aFiles have been reloaded successfully. &7[&b" + names.toString().replace("[", "").replace("]", "").replace(",", "&7,&b") + "&7]&a."));
    }
}
